...
SELECT COUNT(membercost) FROM `Facilities` WHERE membercost = 0.0;
Answer: 4
...
SELECT expense_label, name, monthlymaintenance FROM Facilities ORDER BY expense_label;
...
SELECT DISTINCT
    ...
    CONCAT(m.firstname, " ", m.surname) AS memb_name
FROM Members m
...
SELECT f.name AS fac_name,
    ...
    CASE WHEN b.memid = 0 THEN b.slots*f.guestcost
    	ELSE b.slots*f.membercost END AS tot_cost,
...
WHERE CASE WHEN b.memid = 0 
        ...
        ELSE b.slots*f.membercost END > 30 AND
...
SELECT
	fac_name,
    SUM(subq.book_cost) AS total_revenue
FROM (SELECT
	f.name AS fac_name,
   	CASE WHEN b.memid = 0 THEN b.slots*f.guestcost
    	ELSE b.slots*f.membercost END AS book_cost
	FROM Bookings AS b
	INNER JOIN Facilities as f
	ON b.facid = f.facid) AS subq
GROUP BY fac_name
HAVING SUM(subq.book_cost) < 1000
...
ON b.facid = f.facid
WHERE b.memid <> 0
GROUP BY fac_name, mem_name
ORDER BY fac_name, total_usage DESC;
